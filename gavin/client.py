...
from ctypes import (CFUNCTYPE, POINTER, byref, c_int, c_short, c_ubyte,
                    c_void_p, cast, cdll)
...
def frame_recv_cb(frame):
    ...
    if frame.frame_rgb_data_length > 0:
    ...
    elif frame.frame_src_data_length > 0:
        ...
        handle_frame(img, channel='1', ip='', port=81)
    elif frame.frame_yuv_data_length > 0:
        arr_ptr = cast(frame.frame_yuv_data, POINTER(c_short * frame.frame_yuv_data_length))[0]
        img = frombuffer(arr_ptr, int16).reshape((frame.frame_height, frame.frame_width))
        handle_frame(img, channel='2', ip='', port=82)
    elif frame.paraLine_length > 0:
        arr_ptr = cast(frame.paraLine, POINTER(c_short * frame.paraLine_length))[0]
        img = frombuffer(arr_ptr, int16).reshape((frame.frame_height, frame.frame_width))
        handle_frame(img, channel='3', ip='', port=83)
...
class GAVIN_API(c_void_p):
    ...
    def __call__(self, *args):
        ...
        if ret != 1:
            raise Exception("{} error {} ({})".format(self.name, ret,
                                                      GUIDE_USB_CODE_E(ret).name))
...
class Client(object):
    ...
    def SetPalette(self, index=0):
        ...
        if index not in range(10):
            raise Exception("index value out of range 0-9")
